...
{
    internal sealed record MappedValue(ISourceValue SourceValue, MemberPathGetter? TargetOriginalValueGetter);
    ...
    {
        if (!TryBuildMappedValue(ctx, memberMappingInfo, MemberMappingBuilder.CodeStyle.Expression, out var result))
        {
            sourceValue = null;
            return false;
        }
    ...
    /// <summary>
    /// Tries to build a <see cref="MappedValue"/> containing the source value assignable to the target member
    ...
    /// </summary>
    public static bool TryBuildMappedValue(
        IMembersBuilderContext<IMapping> ctx,
        MemberMappingInfo memberMappingInfo,
        MemberMappingBuilder.CodeStyle codeStyle,
        [NotNullWhen(true)] out MappedValue? result
    )
    {
        if (memberMappingInfo.ValueConfiguration != null)
        {
            ...
            result = new MappedValue(sourceValue, null);
            return true;
        }
