...
internal static class MemberMappingBuilder
{
    ...
    )
    {
        ...
        if (memberMappingInfo.Configuration?.SuppressNullMismatchDiagnostic != true && memberSourceNullable && !memberTargetNullable && !(delegateSourceNullable && !delegateTargetNullable))
        {
            ctx.BuilderContext.ReportDiagnostic(
