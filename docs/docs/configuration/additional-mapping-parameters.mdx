...
## Mapping nested target members from parameters

Additional parameters are matched to flat target members by their (case-insensitive) name.
To map an additional parameter to a nested target member, or to a target member with a
different name, use the `MapPropertyFromParameter` attribute:

<Tabs>
  <TabItem value="parameter-declaration" label="Declaration" default>
    ```csharp
    [Mapper]
    public static partial class ParentMapper
    {
      ...
      // highlight-end
      public static partial ParentDomain ToDomain(ParentDto parent, string parentId, string childId);
    }
    ```
  </TabItem>
  <TabItem value="parameter-generated" label="Generated code">
    ```csharp
    [Mapper]
    public static partial class ParentMapper
    {
...
:::note
To reference a nested target member, the full `nameof` syntax must be used
(e.g. `nameof(@ParentDomain.Child.Id)`). For a top-level target member
`nameof(ParentDomain.Id)` and `nameof(@ParentDomain.Id)` are equivalent.
:::

The source of this mapping can be either the mapping's source object or an additional
parameter, but a target member can only be mapped once. If a target member is configured
by both `MapPropertyFromParameter` and `MapProperty`, Mapperly reports a duplicate
