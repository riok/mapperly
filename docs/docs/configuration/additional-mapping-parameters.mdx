...
## Target original value parameter

A user-implemented mapping method can receive the *current value* of the destination member
being mapped to by marking one parameter with `[MappingTargetOriginalValue]`.
This lets the method use the existing destination value e.g. as a fallback when the source has no value.

<Tabs>
  <TabItem value="destvalue-declaration" label="Declaration" default>
    ```csharp
    ...
    {
      public partial PersonDto MapToDto(Person source);

      // highlight-start
      private static int? FromOptional(
          Optional<int?> source,
          [MappingTargetOriginalValue] int? destination)
  ...
  <TabItem value="destvalue-generated" label="Generated code">
    ```csharp
    public partial PersonDto MapToDto(Person source)
    {
        ...
        // ...
        // highlight-start
        target.Age = FromOptional(source.Age, target.Age);
        // highlight-end
        return target;
    }
    ```
...
:::info
`[MappingTargetOriginalValue]` has the following limitations:
