...
the `RequiredEnumMappingStrategy` can be used. By default this property is set to `Inherit`, which will inherit the value from `RequiredMappingStrategy`.
...
<Tabs>
    <TabItem value="global" label="Global (mapper level)" default>
        ...
        ```csharp
        // highlight-start
        [Mapper(RequiredEnumMappingStrategy = RequiredEnumMappingStrategy.Source)]
        // highlight-end
        public partial class CarMapper
        {
            ...
