...
To apply an existing mapping configuration in the opposite direction, set `Reverse = true` on the `IncludeMappingConfigurationAttribute`.
This swaps the source and target types and reverses all member configurations automatically.
...
The following configurations are reversed:

| Configuration | Reverse behaviour |
| --- | --- |
| `MapProperty` | Source and target member paths are swapped |
| `MapperIgnoreSource` | Becomes `MapperIgnoreTarget` |
| `MapperIgnoreTarget` | Becomes `MapperIgnoreSource` |
| `MapDerivedType` | Source and target types are swapped |
| `MapEnumValue` | Source and target enum values are swapped |
| `MapperRequiredMapping` | Source and target strategies are swapped |

:::info
`MapValue` is not reversed since a constant value has no meaningful inverse.
:::
