...
By default, Mapperly uses the method name as the identifier for each mapper method.
However, you can assign a custom name to any mapper method by applying the `NamedMapping` attribute:
