...
Mapperly also supports referencing methods external to the mapper class to perform property mappings. To do this, prefix the path
with `@` in the `Use` property. This applies for both `MapProperty` and `MapPropertyFromSource`.
It is also possible to use a string reference of the type. In this case the type must be fully qualified.
...
It is also possible to use this syntax to reference instance methods if the mapper class has a property or field referencing the external type.
...
{
    // highlight-start
    [MapProperty(nameof(Car.Price), nameof(CarDto.Price), Use = "OtherNamespace.PriceMapper.CustomMapPrice")]
    [MapPropertyFromSource(nameof(CarDto.NetPrice), Use = "OtherNamespace.PriceMapper.CustomMapPriceFromSource")]
    // highlight-end
    public partial CarDto MapCar(Car source);
...
public static class PriceMapper
{
    ...
    [NamedMapping("CustomMapPriceFromSource")]
    public static string MapPriceFromSource(Car source)
        => (source.Price / 100).ToString("C");
}


### Ignoring members and methods

Specific properties, fields, or methods can be excluded from being used in mappings by applying the `[MapperIgnore]` attribute. This is particularly useful when you want to prevent certain members from being included in automatic mappings.
...
public class Source
{
    ...
    [MapperIgnore]
    public string SecretValue { get; set; } // This will be ignored in mappings
}

### Ignoring methods

You can also ignore specific methods from being used as mapping methods:

```csharp
public class Source
{
    // This method will be ignored by Mapperly
    [MapperIgnore]
    public static Target FromSource(Source src) => new();

    // This method will be used for mapping
    public static Target CreateFrom(Source src) => new();
}
