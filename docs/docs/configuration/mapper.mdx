...
This behavior can be configured via the `StackCloningStrategy` property on the `MapperAttribute`.
...
#### Only explicitly mapped members

By default, Mapperly automatically maps all source members that have a matching target member by name.
Setting `OnlyExplicitMappedMembers` to `true` on the `MapperAttribute` disables this automatic name-based member mapping.
Only members explicitly configured with attributes such as `MapPropertyAttribute` or `MapNestedPropertiesAttribute` will be mapped;
all other members are silently ignored without generating diagnostics.

```csharp
// highlight-start
[Mapper(OnlyExplicitMappedMembers = true)]
// highlight-end
public partial class CarMapper
{
    // Only 'Id' is mapped; all other members are silently ignored
    // highlight-start
...
This is useful when working with types that have many members but only a subset needs to be mapped,
avoiding the need to explicitly ignore every unwanted member.
...
To customize the format provider / culture to be used by Mapperly when calling `ToString` or `Parse`
