...
Sometimes you may want to map a property using custom logic defined outside the mapper class. Mapperly supports referencing such
external methods to perform property mappings. To do this, prefix the path with `@` in the `Use` property. This applies
for both `MapProperty` and `MapPropertyFromSource`. The type containing the method must be accessible in the current context,
either by using a fully qualified path or by including the namespace with `using` directive.
...
<Tabs>
    <TabItem value="fully-qualified-namespace" label="Fully qualified path" default>
        ...
        {
            // highlight-start
            [MapProperty(nameof(Car.Price), nameof(CarDto.Price), Use = nameof(@OtherNamespace.PriceMapper.MapPrice))]
            // highlight-end
            public partial CarDto MapCar(Car source);
...
<Tabs>
    <TabItem value="referenced-through-field" label="Referenced through field" default>
        ...
        {
            // highlight-start
            private readonly PriceMapper _priceMapper = new();
            // highlight-end
            
            // highlight-start
            [MapProperty(nameof(Car.Price), nameof(CarDto.Price), Use = nameof(@_priceMapper.MapPrice))]
            // highlight-end
            public partial CarDto MapCar(Car source);

            // generates
            target.Price = _priceMapper.MapPrice(source.Price);
        }
