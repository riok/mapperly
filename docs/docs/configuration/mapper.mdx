...
| SnakeCase           | Matches properties by converting both names to snake_case before comparison                     |
| UpperSnakeCase      | Matches properties by converting both names to UPPER_SNAKE_CASE before comparison              |

<Tabs>
    <TabItem value="case-insensitive" label="Case insensitive" default>

        Maps properties that differ only in casing.
    ...
    <TabItem value="snake-case" label="Snake case">

        Maps properties by converting both source and target property names to snake_case before comparison.
        ...
        The snake case strategy also supports nested property flattening:
    ...
    <TabItem value="upper-snake-case" label="Upper snake case">

        Maps properties by converting both source and target property names to UPPER_SNAKE_CASE before comparison.
        For example, `FirstName` would match `FIRST_NAME`.

        ```csharp
        // highlight-start
        [Mapper(PropertyNameMappingStrategy = PropertyNameMappingStrategy.UpperSnakeCase)]
        // highlight-end
        public partial class CarMapper
        {
            public partial CarDto ToDto(Car car);
        ...
        The upper snake case strategy also supports nested property flattening:
...
:::note
Property name mapping strategies follow a priority order:
1. Exact name match (fastest)
2. Strategy-specific conversions (snake_case, UPPER_SNAKE_CASE)
3. PascalCase permutations (for backward compatibility)
4. Case variations (for CaseInsensitive strategy)
:::

:::warning
When using a non-PascalCase match, a PascalCase fallback is possible. Both naming conventions will be mapped if they exist.
...
This is a normal behavior when both the `MY_VALUE` and `MyValue` properties are mapped from the same source property `MY_VALUE`.
:::

## Migration from existing strategies

When migrating from `CaseInsensitive` to snake case strategies:

- **Before**: `ModelName` → `modelname`
- **After**: `ModelName` → `model_name` (with SnakeCase) or `MODEL_NAME` (with UpperSnakeCase)

When migrating from `CaseSensitive` to snake case strategies:

- **Before**: Only exact matches work
- **After**: Automatic conversion between PascalCase and snake_case conventions

For existing mappers using explicit `[MapProperty]` attributes, the new strategies provide fallback mappings while maintaining backward compatibility.
