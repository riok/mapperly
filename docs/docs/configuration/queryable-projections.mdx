...
This also applies to standalone [expression mappings](#expression-mappings).
...
Expression mappings share the same limitations, features and configuration options as `IQueryable<T>`
projections, including property configurations, user-implemented mapping methods and
